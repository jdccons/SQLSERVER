...
CREATE PROCEDURE [dbo].[usp_CustIndivRefreshAll] 
...
AS
/* ================================================================
  Object:			usp_CustIndivRefreshAll
  ...
  2016-01-13		3.0			J Criswell		Changed logic on insert statement;
												also added some fields
...
BEGIN TRY
	...
	SET CustomerName = c.CustomerName
		...
		, Rsrv1 = case when c.CreditHold = 'Y' then 3 when c.CreditHold = 'N' then 1 else 1 end
	...
	SELECT c.CustomerKey
		...
		, 9 As GroupType
	FROM vw_IndividualCustomer AS c
	LEFT OUTER JOIN ARCUST_local AS arc
		ON arc.Spare = c.Spare
	WHERE (arc.Spare IS NULL);
...
END CATCH;
GO
EXEC sp_addextendedproperty N'Purpose', N'sync vw_IndividualCustomer to ARCUST_local', 'SCHEMA', N'dbo', 'PROCEDURE', N'usp_CustIndivRefreshAll', NULL, NULL
GO
